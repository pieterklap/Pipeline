...
Run_Repeated_pipeline ()
{
    #   Set backslashes infront of the forward slashes for use in sed
        DSSparam_sed=${DSSparam//\//\\/}
        DSSparam_Repeat_sed=${DSSparam_Repeat//\//\\/}
    #   replace the original parameter location with the new one
        Run_Script_Repeat=$(echo "$Run_Script" | sed "s/$DSSparam_sed/$DSSparam_Repeat_sed/")
...
#   Notes the mass unit (ppm or Daltons)
    MassUnit=$(grep "peptide_mass_units = " $DSSparam | awk '{print $3}')
...
#   Start editing the parameter files
    ...
    #   creates a file which includes the mass tolerance in the name
        DSSparam_Repeat="$DSSparam""_$PrecursorMassTolerence1""_$PrecursorMassTolerence2""$MassUnit"
...
#   Notes the mass unit (ppm or Daltons)
    DSSparam_Default=$(grep "list path, default parameters" $DSSparam | awk -F\> '{print $2}' | awk -F\< '{print $1}')
    MassUnit=$(grep "spectrum, parent monoisotopic mass error units" $DSSparam_Default | awk -F\> '{print $2}' | awk -F\< '{print $1}')
#   Notes the extention of the tandem parameter file if it has one (should be .xml)
    Extention_DSSparam=$(echo $DSSparam | awk -F\/ '{print $NF}' | awk -F\. '{if ($NF > 1) print "."$NF}')
    EXtention_DSSparam_Default=$(echo $DSSparam_Default | awk -F\/ '{print $NF}' | awk -F\. '{if ($NF > 1) print "."$NF}')
...
#   Start edditing parameter file
    ...
    #   creates a file which includes the mass tolerance in the name
        DSSparam_Repeat=${DSSparam//$Extention_DSSparam/_$PrecursorMassTolerence1\_$PrecursorMassTolerence2$MassUnit$Extention_DSSparam}
        DSSparam_Default_Repeat=${DSSparam_Default//$EXtention_DSSparam_Default/_$PrecursorMassTolerence1\_$PrecursorMassTolerence2$MassUnit$EXtention_DSSparam_Default}
        cp $DSSparam $DSSparam_Repeat

    #   changes the peptide mass tolerance
        sed  "s/label=\"spectrum, parent monoisotopic mass error plus\">.*</label=\"spectrum, parent monoisotopic mass error plus\">$PrecursorMassTolerence1.$PrecursorMassTolerence2</" $DSSparam_Default > $DSSparam_Default_Repeat
    #   changes the name of the output to inculde the ppm/daltons
        sed -i "s/label=\"spectrum, parent monoisotopic mass error minus\">.*</label=\"spectrum, parent monoisotopic mass error minus\">$PrecursorMassTolerence1.$PrecursorMassTolerence2</" $DSSparam_Default_Repeat

    #   Escape the forward slash, that way sed can use it.
        DSSparam_Default_sed=${DSSparam_Default//\//\\/}
        DSSparam_Default_Repeat_sed=${DSSparam_Default_Repeat//\//\\/}
    #   replace the location of the standard default parameter file with the repeat default parameter file
        sed -i "s/$DSSparam_Default_sed/$DSSparam_Default_Repeat_sed/"  $DSSparam_Repeat
...
#   Notes the mass unit (ppm or Daltons)
    MassUnit=$(grep "^PrecursorMassTolerance -t " $DSSparam | awk '{print $3}' | awk -F[0-9] '{print $NF}')
    if [[ $MassUnit == "" ]]; then
        MassUnit="ppm"
    fi
...
#   Start editing the parameter files
    ...
    #   changes the peptide mass tolerance and the unit
        sed "s/PrecursorMassTolerance -t .*/PrecursorMassTolerance -t $PrecursorMassTolerence1.$PrecursorMassTolerence2$MassUnit/" $DSSparam > $DSSparam_Repeat
        sed -i "s/#Output_suffix .*/#Output_suffix _$PrecursorMassTolerence1\_$PrecursorMassTolerence2$MassUnit/" $DSSparam_Repeat
...
#   Notes the mass unit (ppm or Daltons)
    MassUnit=$(grep "precursor_mass_units = " $DSSparam | awk '{print $3}')
#   Translates the number in the parameter file to the corresponding unit
    if [[ $MassUnit == "2" ]]; then
        MassUnit="ppm"
    else
...
#   Start editing the parameter files
    ...
    #   creates a file which includes the mass tolerance in the name
        DSSparam_Repeat="$DSSparam""_$PrecursorMassTolerence1""_$PrecursorMassTolerence2""$MassUnit"
    #   changes the peptide mass tolerance and the unit
        sed "s/precursor_mass_tolerance = .*/precursor_mass_tolerance = $PrecursorMassTolerence1.$PrecursorMassTolerence2/" $DSSparam > $DSSparam_repeat
        sed -i "s/#Output_suffix .*/#Output_suffix _$PrecursorMassTolerence1\_$PrecursorMassTolerence2$MassUnit/" $DSSparam_Repeat
